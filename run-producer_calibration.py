...
def run_producer(server={"server": None, "port": None}):
    ...
    soil_epsg_code = int(Path(path_to_soil_grid).stem.rsplit("_", 1)[-1])
    ...
    dem_epsg_code = int(Path(path_to_dem_grid).stem.rsplit("_", 1)[-1])
    ...
    slope_epsg_code = int(Path(path_to_slope_grid).stem.rsplit("_", 1)[-1])
    ...
    crop_epsg_code = int(Path(path_to_crop_grid).stem.rsplit("_", 1)[-1])
    ...
    nuts3_regions_epsg_code = int(Path(path_to_nuts3_regions_grid).stem.rsplit("_", 1)[-1])
                ...
                for srow in range(0, srows):
                    ...
                    for scol in range(0, scols):
                        ...
                        climate_csv_path = (paths["monica-path-to-climate-dir"] +
                                            f"C3S2_414_po_ebro/AgERA5_2014_to_2025/{region}/row-{crow}/col-{ccol}.csv")
